...
<!-- runcmd code: kraken query visualize --help | sed -r "s/\x1B\[([0-9]{1,3}(;[0-9]{1,2})?)?[mGK]//g" -->
```
usage: kraken query visualize [-h] [-v] [-q] [-b PATH] [-p PATH] [--state-name NAME] [--state-dir PATH]
                              [--additional-state-dir PATH] [--no-load-project] [--resume] [--restart {all}] [--all]
                              [-i] [-s] [-R] [-r]
                              [task ...]

generate a GraphViz of the build
...
visualization options:
  -i, --inactive               include inactive tasks in the graph
  -s, --show                   show the graph in the browser (requires dot)
  -R, --reduce                 fully transitively reduce the graph
  -r, --reduce-keep-explicit   transitively reduce the graph but keep explicit edges
```
<!-- end runcmd -->
