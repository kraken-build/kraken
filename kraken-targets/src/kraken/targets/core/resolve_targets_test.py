...
def test__resolve_targets(kraken_ctx: Context, kraken_project: Project, runner: RuleRunner) -> None:
    ...
    target = create_target("mytarget", project, None, inspect.currentframe())

    request = ResolveTargetsRequest(addresses=(Address("mytarget"),), relative_to=project.address, transitive=False)
