...
class UvIndexes:
    ...
    def to_config(self) -> list[dict[str, Any]]:
        ...
        for index in self.indexes:
            entry: dict[str, Any] = {"url": index.url}
            if len(index.name) > 0:
    ...
    def to_env(self) -> dict[str, str]:
        """Convert UV configuration for indexes into a environment variables."""
        ...
        for index in self.indexes:
            if index.default:
                ...
                uv_indexes.append((f"{index.name}=" if index.name != "" else "") + index.unsafe_url)
...
class UvPyprojectHandler(PyprojectHandler):
    ...
    def set_package_indexes(self, indexes: Sequence[PackageIndex]) -> None:
        ...
        self.raw.setdefault("tool", {}).setdefault("uv", {})["index"] = []
